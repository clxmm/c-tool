...
1. 在输入框中粘贴 JSON 字符串
2. 点击「格式化」按钮进行格式化
3. 如果 JSON 格式正确，右侧会显示格式化后的结果
4. 点击「复制」可复制结果到剪贴板

### 时间戳转换

- **当前时间**：页面顶部实时显示当前时间（精确到毫秒）和时间戳
- **时间戳 → 日期**：输入时间戳（支持秒或毫秒），点击转换获取日期时间
- **日期 → 时间戳**：选择日期时间，自动转换为秒级和毫秒级时间戳

### Base64 转换

- **文本转 Base64**：输入文本，点击转换获取 Base64 编码
- **Base64 转文本**：输入 Base64 字符串，点击转换获取原始文本
- **图片转 Base64**：拖拽或点击上传图片，自动转换为 Base64 格式

### 主题切换

点击页面右上角的主题开关，可在明亮/暗黑模式之间切换，设置会自动保存到本地存储。
